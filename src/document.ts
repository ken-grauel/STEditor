...
export class Document {
	...
	validateAll(): void {
		let index = 0
		while (index <this._lines. length &&this._lines[index].isValidated)
			index ++

		for (; index <this._lines. length; index ++){
			this._lines[index]. invalidate ();
			const tokenizerCarryover= (index ===0? null : (this._lines [index -1]. tokenizerResult as TokenizerResult). carryoverState)
			const tokenizerResult= this._language. tokenizer (
				this._lines [index]. text, tokenizerCarryover)
	...
	debugPrint (callback: (output: string) => void): void {
		for (let line of this._lines) {
			...
			if (line.isValidated) {
				...
				callback (tokenizerResult. tokens. map (
					token => "[" + line. text.substr(token. index, token. length) + "]/" + TokenType[token. type]
				). join (" "))
			}
		}
	}
